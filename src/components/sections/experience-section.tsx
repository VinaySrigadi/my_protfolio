        ...
        <div className="relative border-l border-border ml-4 md:ml-0">
          {experiences.map((exp, idx) => (
            <FadeIn key={idx} delay={idx * 0.1} direction="up" className="mb-12 last:mb-0 relative pl-8 md:pl-12">
              {/* Timeline dot */}
              <div className="absolute left-[-5px] top-1 h-3 w-3 rounded-full bg-primary border-4 border-[#09090B] box-content shadow-[0_0_10px_rgba(37,99,235,0.5)]" />
              ...
              <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 mb-4">
                ...
                <div className="flex items-center gap-2 text-sm text-muted-foreground bg-muted/30 border border-border px-3 py-1.5 rounded-full whitespace-nowrap self-start">
              ...
              <ul className="space-y-3 text-muted-foreground leading-relaxed mb-6">
                {exp.description.map((highlight, hIdx) => (
                  <li key={hIdx} className="relative pl-5 before:content-[''] before:absolute before:left-0 before:top-2.5 before:w-1.5 before:h-1.5 before:rounded-full before:bg-primary/60">
                    {highlight}
                  </li>
                ))}
              </ul>
